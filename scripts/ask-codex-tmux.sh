...
if $USE_EXEC; then
    ...
    tmux send-keys -t "$SESSION:$WINDOW.$TARGET_PANE" "codex --dangerously-bypass-approvals-and-sandbox exec '$ESCAPED_PROMPT' --skip-git-repo-check -o '$OUTPUT_FILE' 2>&1 | tee -a '$OUTPUT_FILE.log'; echo '=== CODEX_DONE ===' >> '$OUTPUT_FILE.log'"
    sleep 0.5
    ...
    tmux send-keys -t "$SESSION:$WINDOW.$TARGET_PANE" "codex --dangerously-bypass-approvals-and-sandbox '$ESCAPED_PROMPT'"
    sleep 0.5
    tmux send-keys -t "$SESSION:$WINDOW.$TARGET_PANE" Enter
